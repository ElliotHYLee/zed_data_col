...
int main(int argc, char **argv) {
    ...
    init_params.depth_mode = DEPTH_MODE_MEDIUM;//DEPTH_MODE_PERFORMANCE;
            ...
            if (tracking_state == TRACKING_STATE_OK)
            {
              if (index >=20)
              {
                ...
                if (1000000000.0/dt < 11)
                {
                  ...
                  cv::imwrite("../Data/left/" + to_string(dataNum) + ".jpg", image_ocv_left);
                  cv::imwrite("../Data/right/" + to_string(dataNum) + ".jpg", image_ocv_right);
                  dataNum++;
                }


                //cv::imwrite("../Data/depth/" + to_string(index-20) + ".jpg", depth_image_ocv);
                // Handle key event
                key = cv::waitKey(10);
              }
              index++;
